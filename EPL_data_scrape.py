...
# All final calls to retrieve data as dataframes
fix = get_fixtures()
fix = fix[fix['Score'].notna()]
# find the current match week from the fixtures table
games = fix.groupby('Wk').size()
# label the most recent week as current
week = f"W{int(games.index.max())}"
# establish directory variable
directory = "~/Desktop/EPL-Scrape/"

# if match week folder already exists, we perform no web-scraping
if os.path.isdir(f"{directory}{week}"):
    print("Matchweek folder already created")

# if the number of games in the most recent match week is 10 (all games played) then we scrape data
# output all data (fixtures, league table, players, team tables, keeper tables) to csv files
elif games[games.index.max()] == 10:
    make_folder = os.path.join(directory, week)
    os.mkdir(make_folder)
    league_table = get_league_table()
    fix.to_csv(f"{directory}{week}//EPL_Fixtures_{week}.csv")
    league_table.to_csv(f"{directory}{week}/EPL_LT_{week}.csv", index=False)
    ...
    df_outfield.to_csv(f"{directory}{week}/EPL_players_{week}.csv", index=False)
    ...
    df_team1.to_csv(f"{directory}{week}/EPL_teams_{week}.csv", index=False)
    df_keeper = get_keeper_data('https://fbref.com/en/comps/9/', '/Premier-League-Stats')
    df_keeper.to_csv(f"{directory}{week}/EPL_keepers_{week}.csv", index=False)

# if most recent match week has less than 10 games, we do not web-scrape data
else:
    print(f'Matchweek {week} not yet complete')
